...
export const loginUser = async (data: { username: string; password: string }) => {
  const response = await apiClient.post("/User", data); // Adjusting to `/api/User` for login
...
export const signUpUser = async (data: { username: string; password: string }) => {
  const response = await apiClient.post("/Auth", data); // Adjusting to `/api/Auth` for sign-up
  return response.data; // Return response payload
};